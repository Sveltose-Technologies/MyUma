...
const BrowseDetails = () => {
  ...
  const [newMessage, setNewMessage] = useState("");
  const [isSending, setIsSending] = useState(false);
  const [chatUsers, setChatUsers] = useState([]); // Threads for the owner
  const [selectedUser, setSelectedUser] = useState(null); // Client selected by owner
  const [viewMode, setViewMode] = useState("list"); // 'list' for owner inbox, 'chat' for conversation
...
const submitInquiry = async (e) => {
  ...
  try {
    const payload = {
      fullName: inquireForm.fullName,
      email: inquireForm.email,
      phoneNo: inquireForm.phoneNo,
      comment: inquireForm.comment,
      itemId: listing._id, // Ensure this is just the string ID
    };

    console.log("Sending Inquiry Payload:", payload); // Debugging ke liye

    const res = await sendInquireApi(payload);

    // Server success response check
    if (res.success === true || res.status === 200) {
      toast.success("Inquiry sent successfully!");
      setShowInquireModal(false);
      setInquireForm({ fullName: "", email: "", phoneNo: "", comment: "" });
    } else {
      toast.error(res.message || "Failed to send inquiry.");
    }
  } catch (error) {
    console.error("Inquiry Error:", error.response?.data);
    // 400 error ka exact reason message dikhayega
    toast.error(
      error.response?.data?.message || "Fields are missing or invalid (400)",
    );
  } finally {
  ...
  // --- CHAT LOGIC ---
const fetchChatData = async () => {
  // REQ FIX: Listing ya Owner load hone ka wait karein
  if (!isLoggedIn || !listing || !listing.ownerId?._id) return;
    ...
    if (isOwnerOfListing) {
      ...
      if (res.success) {
        ...
        res.data.forEach((msg) => {
          const otherUser =
            msg.senderId === currentId ? msg.receiverData : msg.senderData;
          if (otherUser && otherUser._id && !map.has(otherUser._id)) {
            map.set(otherUser._id, true);
            uniqueUsers.push(otherUser);
          }
        });
    ...
    <div className="bg-light min-vh-100 pt-4 pb-5">
      ...
      <div className="container mt-4">
        <div className="row g-4">
          ...
          <div className="col-lg-4 col-12">
            ...
            <div className="card border-0 shadow-sm rounded-4 p-4 mb-4 bg-white border text-center">
              ...
              {!isOwnerOfListing && (
                <button
                  ...
                  className="btn uma-btn-navy w-100 mt-4 rounded-pill d-flex align-items-center justify-content-center gap-2 fw-bold py-2 shadow-sm"
                  style={{
                    backgroundColor: "#001f3f",
                    color: "white",
                    border: "none",
                  }}>
                  <ClipboardPen size={18} /> INQUIRE NOW
                </button>
              )}
            ...
            <div className="card border-0 shadow-sm rounded-4 p-4 bg-white border">
              <h6
                className="fw-800 mb-3 text-navy text-uppercase text-center"
                style={{ fontSize: "11px" }}>
                Connect with Business
              </h6>
              <div className="d-flex flex-wrap gap-2 justify-content-center">
      ...
      {showInquireModal && (
        <div
          ...
          <div
            className="bg-white rounded-4 shadow-lg overflow-hidden animate-fade-in"
            ...
            <div
              ...
              <h6 className="m-0 fw-bold d-flex align-items-center gap-2">
                <ClipboardPen size={18} /> Send Inquiry
              </h6>
              <X
                className="cursor-pointer"
            ...
            <form onSubmit={submitInquiry} className="p-4 bg-light">
              <div className="mb-3">
                ...
                <input
                  type="text"
                  className="form-control rounded-pill border-0 shadow-sm px-3"
                  value={inquireForm.fullName}
                  onChange={(e) =>
                    setInquireForm({ ...inquireForm, fullName: e.target.value })
                  }
                  required
                />
              </div>
              ...
              <div className="mb-3">
                ...
                <input
                  ...
                  className="form-control rounded-pill border-0 shadow-sm px-3"
                  placeholder="+1 234..."
                  value={inquireForm.phoneNo}
                  onChange={(e) =>
                    setInquireForm({ ...inquireForm, phoneNo: e.target.value })
                  }
                  required
                />
              </div>
              <div className="mb-4">
                ...
                <textarea
                  className="form-control border-0 shadow-sm px-3 pt-2"
                  style={{ borderRadius: "15px" }}
                  rows="3"
                  value={inquireForm.comment}
                  onChange={(e) =>
                    setInquireForm({ ...inquireForm, comment: e.target.value })
                  }
                  required
                />
              </div>
              <button
                ...
                className="btn uma-btn-navy w-100 rounded-pill fw-bold py-2 shadow-sm d-flex align-items-center justify-content-center gap-2"
