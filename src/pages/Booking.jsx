...
import { toast } from "react-toastify";
import { Trash2, Eye, MapPin, X, Calendar } from "lucide-react";
...
const Bookings = () => {
  ...
  // Get current user from localStorage to filter bookings
  ...
  const fetchBookings = async () => {
    try {
      ...
      const res = await getAllBookingsAPI();
      if (res.status) {
        // Filter bookings where the userId matches the logged-in user
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <span className="badge bg-danger rounded-pill px-3 py-2">
          {bookings.length} Bookmarks Found
        </span>
      </div>
          ...
          <table className="table table-hover align-middle mb-0">
            <thead className="bg-light">
              <tr>
                ...
                <th className="px-4 py-3 text-secondary border-0">Title</th>
                <th className="px-4 py-3 text-secondary border-0">Address</th>
                <th className="px-4 py-3 text-secondary border-0">Date</th>
                <th className="px-4 py-3 text-secondary border-0 text-center">
                  Actions
                </th>
              </tr>
            </thead>
      ...
      {selectedInfo && (
        <div
          ...
          style={{ backgroundColor: "rgba(0,0,0,0.5)", zIndex: 9999 }}>
          <div
            className="bg-white rounded-4 shadow-lg p-4 w-100 m-3"
            ...
            <button
              className="btn btn-navy w-100 mt-3 rounded-pill text-white fw-bold"
              style={{ backgroundColor: "#001f3f" }}
              onClick={() => setSelectedInfo(null)}>
              Close
            </button>
