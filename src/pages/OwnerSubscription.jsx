  ...
  const fetchSubscriptionData = async () => {
    ...
    try {
      ...
      if (res?.success && res.payments?.length > 0) {
        ...
        const latestSuccessful = sortedHistory.find(
          (p) => p.status === "success",
        );

        if (latestSuccessful) {
          const planMeta = availablePlans.find(
            (p) => p.name === latestSuccessful.planName,
          );
        ...
        <div className="col-12">
          <div className="mb-4">
            ...
            <p className="text-muted small">
              View your current active plan and billing history.
            </p>
          </div>
            ...
            <div className="col-12">
              {activeSub ? (
                        ...
                        <div className="mt-2 position-relative">
                          ...
                          <div className="row g-3 bg-light rounded-3 p-3 m-0">
                            <div className="col-md-6 border-end">
                              <small className="text-muted d-block extra-small fw-bold">
                                ACTIVATED ON
                              </small>
                              <span className="fw-bold text-navy">
                                {activeSub.startDate.toLocaleDateString(
                                  "en-GB",
                                )}
                              </span>
                            </div>
                            <div className="col-md-6 ps-md-4">
                              <small className="text-muted d-block extra-small fw-bold text-danger">
                                EXPIRES ON
                              </small>
                              <span className="fw-bold text-danger">
                                {activeSub.endDate.toLocaleDateString("en-GB")}
                              </span>
                            </div>
                          </div>
                        </div>
                ...
                <div className="card border-0 shadow-sm rounded-4 p-5 text-center bg-white mb-4">
                  ...
                  <button
                    className="btn btn-warning px-4 py-2 rounded-pill fw-bold mt-2 shadow-sm"
                      ...
                      {currentItems.length > 0 ? (
                        currentItems.map((item, idx) => (
                          <tr key={item._id}>
                            ...
                            <td>
                              <span
                                className={`badge rounded-pill px-3 py-1 small ${item.status === "success" ? "bg-success-subtle text-success border border-success-subtle" : "bg-warning-subtle text-warning border border-warning-subtle"}`}>
