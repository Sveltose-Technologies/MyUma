...
const HomeSearchBar = () => {
  ...
  // --- KEYWORD SUGGESTIONS (Search in Titles & Categories) ---
  ...
  // --- FREE LOCATION API (Nominatim OpenStreetMap) ---
  ...
  // --- SEARCH LOGIC (ALWAYS GO TO BROWSE) ---
  const handleSearch = (overrideCategory) => {
    ...
    // Navigate to Browse page with state (Filters will be applied there)
    navigate("/browse", {
      state: {
      ...
      <div className="row justify-content-center">
        <div className="col-12 col-xl-11">
          ...
          <div className="d-flex flex-wrap justify-content-center gap-2 mt-4">
            {/* 1. STATIC BADGE: Business Directory (Always Visible) */}
            <div
              className="bg-navy px-4 py-2 rounded-pill d-flex align-items-center gap-2 shadow-sm transition-hover border-0"
              style={{ cursor: "pointer" }}
              onClick={() => handleSearch("Business Directory")}>
              <Briefcase size={15} className="text-tan" />
              <span className="text-white extra-small fw-800 ls-1">
                BUSINESS DIRECTORY
              </span>
            </div>

            {/* 2. DYNAMIC BADGES: Other Featured Categories from Database */}
            {featuredBadges
              .filter((cat) => cat.name?.toLowerCase() !== "business directory") // Prevent duplicate if it exists in DB
              .map((item) => (
                <div
                  key={item._id}
