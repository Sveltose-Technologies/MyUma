...
//   // RED LINE FIX: Function ko useEffect ke andar rakha hai
//   useEffect(() => {
...
//         }
//       } catch (error) {
//         console.error("Fetch Error:", error); // Catch block ki red line fix
//       }
//     };

//     getprofileHandler();
//   }, []); // Ab yahan [] khali rakhne par koi red line nahi aayegi
...
//       }
//     } catch (error) {
//       console.error("Update Error:", error); // Catch block ki red line fix
...
const ProfileUpdate = () => {
  ...
  // 'role' ki red line hatane ke liye ise neeche logic mein use kiya gaya hai
  const [role, setRole] = useState("owner");
  const [dbImage, setDbImage] = useState(""); 
  const [selectedFile, setSelectedFile] = useState(null); 
  const [previewImage, setPreviewImage] = useState(null); 
    ...
    const getprofileHandler = async () => {
      ...
      try {
        ...
        if (profile) {
          ...
          setRole(profile.role || "owner"); // API se role set karein
          setDbImage(profile.profileImage || "");
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append("country", formData.country);
    data.append("status", formData.status);
    data.append("role", role); // Hardcoded "owner" ki jagah variable use kiya
                ...
                <div className="col-md-4 bg-navy text-white text-center p-4 d-flex flex-column align-items-center justify-content-center">
                  <div className="position-relative d-inline-block mb-3 mt-4">
                    <img
                      ...
                      style={{ width: "150px", height: "150px", objectFit: "cover" }}
                    />
                    <label htmlFor="avatarUpload" className="position-absolute bottom-0 end-0 bg-tan rounded-circle p-2 shadow border border-white" style={{ cursor: "pointer", width: "45px", height: "45px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <span className="text-navy">📷</span>
                      <input type="file" id="avatarUpload" hidden onChange={handleImageChange} accept="image/*" />
                    </label>
                  </div>
                  <h4 className="fw-800 mb-1 px-2">{formData.fullName || "Owner Name"}</h4>
                  <p className="small text-tan fw-bold text-uppercase ls-1">
                    {role === "owner" ? "Business Owner" : role} {/* Variable use karne se red line hat jayegi */}
                  </p>
                  <hr className="my-3 w-50 opacity-25" />
                  <div className={`badge rounded-pill px-3 py-2 ${formData.status === 'active' ? 'bg-success' : 'bg-danger'}`}>
                    {formData.status.toUpperCase()}
                ...
                <div className="col-md-8 bg-white p-4 p-md-5">
                  ...
                  <form onSubmit={handleSubmit}>
                    <div className="row g-3">
                      <div className="col-md-6">
                        <label className="small fw-800 text-muted mb-1 text-uppercase ls-1">Full Name</label>
                        <input type="text" name="fullName" className="form-control bg-light border-0 py-2 shadow-sm" value={formData.fullName} onChange={handleInputChange} />
                      </div>
                      <div className="col-md-6">
                        <label className="small fw-800 text-muted mb-1 text-uppercase ls-1">Contact Number</label>
                        <input type="text" name="contactNo" className="form-control bg-light border-0 py-2 shadow-sm" value={formData.contactNo} onChange={handleInputChange} />
                      </div>
                      <div className="col-md-12">
                        <label className="small fw-800 text-muted mb-1 text-uppercase ls-1">Email Address</label>
                        <input type="email" name="email" className="form-control bg-light border-0 py-2 shadow-sm" value={formData.email} onChange={handleInputChange} />
                      </div>
                      <div className="col-md-6">
                        <label className="small fw-800 text-muted mb-1 text-uppercase ls-1">City</label>
                        <input type="text" name="city" className="form-control bg-light border-0 py-2 shadow-sm" value={formData.city} onChange={handleInputChange} />
                      </div>
                      <div className="col-md-6">
                        <label className="small fw-800 text-muted mb-1 text-uppercase ls-1">Country</label>
                        <input type="text" name="country" className="form-control bg-light border-0 py-2 shadow-sm" value={formData.country} onChange={handleInputChange} />
                      </div>
                      <div className="col-12">
                        <label className="small fw-800 text-muted mb-1 text-uppercase ls-1">Full Address</label>
                        <textarea name="address" className="form-control bg-light border-0 shadow-sm" rows="3" value={formData.address} onChange={handleInputChange}></textarea>
                      </div>
                    </div>
                    <div className="mt-5">
                      <button type="submit" className="uma-btn-navy w-100 fw-800 border-0 shadow">UPDATE OWNER PROFILE</button>
                    </div>
                  </form>
                </div>
              </div>
