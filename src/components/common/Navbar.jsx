  ...
  return (
    ...
    <nav className="navbar navbar-expand-lg bg-navy sticky-top py-3 shadow w-100 border-0">
      <div className="container px-4">
        ...
        {/* Sidebar Container */}
        <div className="offcanvas offcanvas-end bg-navy border-0" id="navbarOffcanvas" tabIndex="-1">
          <div className="offcanvas-header px-4 pt-4">
            <h5 className="offcanvas-title text-white fw-bold">
              My<span className="text-tan">Uma</span>
            </h5>
          ...
          <div className="offcanvas-body">
            <ul className="navbar-nav mx-auto text-start text-lg-center mb-4 mb-lg-0">
              <li className="nav-item">
                <button onClick={() => handleNavigation("/")} className="nav-link-uma bg-transparent border-0 w-100 text-start text-lg-center">Home</button>
              </li>
              <li className="nav-item">
                <button onClick={() => handleNavigation("/pricing")} className="nav-link-uma bg-transparent border-0 w-100 text-start text-lg-center">Pricing</button>
              </li>
              <li className="nav-item">
                <button onClick={() => handleNavigation("/blog")} className="nav-link-uma bg-transparent border-0 w-100 text-start text-lg-center">Blog</button>
              </li>
              <li className="nav-item">
                <button onClick={() => handleNavigation("/contact")} className="nav-link-uma bg-transparent border-0 w-100 text-start text-lg-center">Contact us</button>
              </li>
            </ul>
              ...
              {token ? (
                <div className="d-flex flex-column flex-lg-row gap-2 w-100 align-items-stretch align-items-lg-center">
                  ...
                  <div className="dropdown flex-grow-1">
                    ...
                    <ul className="dropdown-menu dropdown-menu-end shadow-lg border-0 mt-2 py-0 overflow-hidden">
                      <li><button className="dropdown-item py-2 border-bottom" onClick={() => handleNavigation("/listing")}>My Listing</button></li>
                      <li><button className="dropdown-item py-2 border-bottom" onClick={() => handleNavigation("/reviews")}>Reviews</button></li>
                      <li><button className="dropdown-item py-2 border-bottom" onClick={() => handleNavigation("/profile")}>My Profile</button></li>
                      <li><button className="dropdown-item py-2 text-danger fw-bold bg-light" onClick={handleLogout}>Logout</button></li>
                    </ul>
                  </div>

                  <button onClick={() => handleNavigation("/listing")} className="uma-btn uma-btn-primary py-2 px-3" style={{ fontSize: "12px" }}>New Listing</button>
                  <button onClick={() => handleNavigation("/browse")} className="uma-btn uma-btn-primary py-2 px-3" style={{ fontSize: "12px" }}>Browse Listings</button>
                </div>
              ) : (
                <button onClick={() => handleNavigation("/login")} className="uma-btn uma-btn-primary w-100 w-lg-auto">Sign In</button>
              )}
            </div>
          </div>
        </div>
